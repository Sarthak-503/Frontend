import { Injectable } from '@angular/core';
import { Observable } from 'rxjs';
import { HttpClient } from '@angular/common/http';
import { environment } from '../../environments/environment';
// User interface
export class User {
  name!: String;
  email!: String;
  password!: String;
  password_confirmation!: String;
} 
export class User1 {
  email!: String;
  password!: String;
} 
export class User2{
  email!:String;
}
@Injectable({
  providedIn: 'root',
})
export class AuthService {
  constructor(private http: HttpClient) {}
  // User registration
  register(user: User): Observable<any> {
    return this.http.post(environment.API_URL + 'register', user);
  }
  // Login(1)
  signin(user: User): Observable<any> {
    return this.http.post<any>(environment.API_URL + 'login', user);
  }
  // Access user profile
  profileUser(): Observable<any> {
    
    return this.http.get(environment.API_URL + 'profile');
  }
  //2
  sendResetPasswordLink(user: User) 
  {
    return this.http.post(environment.API_URL + 'forget-password', user); 
  }
  
   
  resetPassword(user:User) {
    return this.http.post(environment.API_URL + 'reset-password', user);
  }

  changeadminpassword(user:User){
    return this.http.put(environment.API_URL + 'user/settings/change-password', user);
  }
}